...
void FusionAhrsUpdate(
    ...
    if ((fabsf(gx) > ahrs->gyroscopeRange) ||
        ...
        // Preserve quaternion, reset internal state (mirrors FusionAhrsRestart)
    ...
    // SECTION 10: ZERO HEADING (only during init)
    if (ahrs->initialising) {
...
int main(int argc, char* argv[]) {
    ...
    // Default to test_10_samples.csv
    const char* input_path = "test_10_samples.csv";
    const char* output_path = "test_10_output.csv";
    bool use_csv = true;
    
    // Check command line arguments
    if (argc > 1) {
        input_path = argv[1];
        // Detect if it's CSV or binary based on extension
    ...
    if (use_csv) {
        ...
        printf("Loaded %d samples from %s (CSV format)\n\n", num_samples, input_path);
    } else {
        ...
        printf("Loaded %d samples from %s (binary format)\n\n", num_samples, input_path);
    }
