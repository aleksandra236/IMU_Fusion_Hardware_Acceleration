 ...
 * @brief CPU Module
 *
 * Orchestrates the AHRS processing pipeline each batch exactly as in the
 * C++ spec (main_profile_realtime.cpp):
 *
 *   Loop:
 *     1. Wait for sensor_ready  (SPI filled BRAM_RAW_OFFSET with 10 raw samples)
 *     2. SW Phase 1 – for each raw sample run SW_PrepareSample (Sections 1-4):
 *          - Store accelerometer
 ...
 *     5. SW Phase 2 – for each HW output sample run SW_ProcessOutput (Sections 9-11):
 *          - Quaternion normalise (fast inverse sqrt)
 *          - Zero-heading correction (during initialisation)
 *          → Update internal quaternion state
 *     6. Store final quaternion of the batch to DDR
 *     7. Repeat until MAX_BATCHES, then stop.
 *
 * Socket layout:
...
private:
    ...
    int batch_count;  // track how many batches have been processed, to stop after MAX_BATCHES
};
