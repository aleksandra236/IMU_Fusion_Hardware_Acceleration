...
// Address is a word offset into the raw_buffer (0 .. HW_BATCH_SIZE*RAW_SENSOR_WORDS-1)
// ============================================================================
void SPI::b_transport(tlm::tlm_generic_payload &pl, sc_core::sc_time &offset)
{
    ...
    if (cmd == tlm::TLM_READ_COMMAND) {
        if (addr < (uint32_t)(HW_BATCH_SIZE * RAW_SENSOR_WORDS)) {
...
void SPI::spi_process()
{
    ...
    while (true) {
        ...
        if (pl.get_response_status() != tlm::TLM_OK_RESPONSE) {
            std::cout << "[SPI] Error: sensor read failed at batch_idx=" << batch_idx << std::endl;
            continue;
        }

        // ── Step 3: Store raw sample in internal buffer ──────────────────────
        for (int w = 0; w < RAW_SENSOR_WORDS; w++) {
            raw_buffer[batch_idx * RAW_SENSOR_WORDS + w] = raw_buf[w];
        }

        batch_idx++;

        // ── Step 4: Batch complete? ──────────────────────────────────────────
        if (batch_idx >= HW_BATCH_SIZE) {
            batch_idx = 0;
            total_batches++;

            std::cout << "[SPI] Batch " << total_batches
                      << " raw data buffered. Notifying CPU..." << std::endl;
